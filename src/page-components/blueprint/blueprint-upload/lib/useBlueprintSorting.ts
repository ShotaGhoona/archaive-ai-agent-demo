...
export function useBlueprintSorting() {
  ...
  const addFilesToProject = useCallback(
    (projectId: string, files: UploadedFile[]) => {
      const newAssignments: ProjectAssignment[] = files.map((file) => ({
        fileId: file.id,
        projectId: projectId,
        assignedAt: new Date(),
  ...
  // プロジェクトから図面を削除
  const removeFilesFromProject = useCallback(
    (projectId: string, fileIds: string[]) => {
      setState((prev) => ({
        ...prev,
        assignments: prev.assignments.filter(
          (a) => !(a.projectId === projectId && fileIds.includes(a.fileId)),
        ),
        projects: prev.projects.map((project) =>
          project.id === projectId
            ? {
                ...project,
                fileCount: Math.max(0, project.fileCount - fileIds.length),
              }
            : project,
        ),
      }));
  ...
  // ドラッグアイテムから図面リストを取得
  const getFilesFromDragItem = useCallback(
    (dragItem: DragItem): UploadedFile[] => {
  ...
  // ドロップ処理：新プロジェクト作成
  const handleDropToNewProject = useCallback(
    (dragItem: DragItem) => {
      ...
      state.projects.forEach((project) => {
        ...
        const filesToRemove = fileIds.filter((id) =>
          projectFileIds.includes(id),
        );
      ...
      if (dragItem.type === 'stack') {
        setState((prev) => ({
          ...prev,
          fileStacks: prev.fileStacks.filter(
            (stack) => stack.id !== dragItem.id,
          ),
          selectedStacks: prev.selectedStacks.filter(
            (id) => id !== dragItem.id,
          ),
        }));
      }
  ...
  const handleDropToProject = useCallback(
    (dragItem: DragItem, targetProjectId: string) => {
      const files = getFilesFromDragItem(dragItem);
      if (files.length === 0) return;
