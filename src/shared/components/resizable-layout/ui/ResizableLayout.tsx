...
export function ResizableLayout({
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      if (!isDragging || !containerRef.current || dragHandleIndex === undefined)
        return;
      ...
      if (config.direction === 'horizontal') {
        ...
        const totalChange =
          newLeftSize -
          newSizes[dragHandleIndex] +
          (newRightSize - newSizes[dragHandleIndex + 1]);
        ...
        const totalChange =
          newTopSize -
          newSizes[dragHandleIndex] +
          (newBottomSize - newSizes[dragHandleIndex + 1]);
        if (Math.abs(totalChange) < 0.1) {
  ...
  const handleSetIsDragging = useCallback(
    (dragging: boolean, handleIndex?: number) => {
  ...
  const layoutClass =
    config.direction === 'horizontal' ? 'flex h-full' : 'flex flex-col h-full';
