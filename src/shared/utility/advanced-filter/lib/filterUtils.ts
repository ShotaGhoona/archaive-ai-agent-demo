...
export function applyFilters<T>(
  ...
  return data.filter((item) => {
    return config.every((filterConfig) => {
      ...
      const itemValue = getValue(item, filterConfig.key as string);
      ...
      if (
        ...
        filterConfig.type === 'datetime-local'
      ) {
...
export function hasActiveFilters<T>(
  ...
  return Object.keys(filters).some((key) => {
    ...
    if (typeof current === 'object' && current !== null) {
      ...
      return (
        currentRange.from !== initialRange.from ||
        currentRange.to !== initialRange.to
      );
    }
