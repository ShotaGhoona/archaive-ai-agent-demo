...
export function PreviewTable({ columns }: PreviewTableProps) {
  ...
  // 動的にTableConfigを作成
  const createPreviewTableConfig = (): TableViewConfig<PreviewRecord> => ({
    columns: [
      // 操作列を追加
      {
        key: 'actions',
      ...
      ...visibleColumns.map((col) => ({
        ...
        editable: false,
        locked: true,
        sortType: col.dataType === 'number' ? 'number' as const : col.dataType === 'date' ? 'date' as const : 'string' as const,
        render: (item: PreviewRecord, value: unknown) => {
          // select型の表示
    ...
    pagination: {
      ...
      allowedItemsPerPage: [5, 10, 20],
      showItemsPerPageSelector: false,
      maxVisiblePages: 3,
    },
  });

  const config = createPreviewTableConfig();
