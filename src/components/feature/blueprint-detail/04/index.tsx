...
import LargeModalChat from "./LargeModalChat";
import { Message } from "./types";

// ダミー応答生成
const generateDummyResponse = (userMessage: string): string => {
  const responses = [
    "詳細な技術相談ありがとうございます。大画面モードでより詳しくお答えできます。",
    "図面の仕様について確認いたします。どの部分について具体的にお知りになりたいですか？",
    "この材質・加工方法について、専門的な観点からお答えします。",
    "製造工程での注意点や最適化について詳しく説明いたします。",
    "技術資料と照合しながら、正確な情報をお提供します。",
  ];
  
  if (userMessage.includes("材質")) {
    return "この図面で使用されている材質について詳しく説明します。材質の特性、加工特性、強度などの技術データを含めてお答えできます。具体的にどの点について知りたいですか？";
  }
  if (userMessage.includes("寸法")) {
    return "寸法に関するご質問ですね。図面上の寸法公差、測定方法、品質管理のポイントについて詳しく解説いたします。どの寸法についてご相談でしょうか？";
  }
  if (userMessage.includes("工程")) {
    return "製造工程についてのご相談ですね。最適な加工順序、使用する工具、品質チェックポイントなど、工程全体を通じた技術的なアドバイスをお提供します。";
  }
  if (userMessage.includes("強度")) {
    return "強度計算や構造解析についてお答えします。応力集中、安全率、疲労強度など、設計に必要な技術的な検討事項について詳しく説明できます。";
  }
  
  return responses[Math.floor(Math.random() * responses.length)];
};

export default function BlueprintDetail04() {
  ...
  const handleQuickAction = useCallback((action: string) => {
    const actionMessage = `${action}機能を実行します。大画面モードでより詳細な操作が可能です。`;
    ...
    <>
      <div className="flex gap-6 h-[calc(100vh-45px)] p-6">
        ...
        <div className="flex-1 flex flex-col space-y-4 min-h-0">
          ...
          {/* メイン図面表示 */}
          <div className="relative flex-1 bg-gray-50 rounded-lg overflow-hidden min-h-0">
            <img
