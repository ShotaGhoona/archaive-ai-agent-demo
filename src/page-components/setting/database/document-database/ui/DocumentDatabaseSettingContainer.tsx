...
export function DocumentDatabaseSettingContainer() {
  ...
  const [selectedCategoryId, setSelectedCategoryId] =
    useState<string>('project-related');
  ...
  const handleUpdateColumn = (
    ...
    setState((prev) =>
      DocumentDatabaseService.updateColumn(prev, typeId, columnId, updates),
    );
  };
  ...
  const handleDeleteColumn = (typeId: string, columnId: string) => {
    setState((prev) =>
      DocumentDatabaseService.deleteColumn(prev, typeId, columnId),
    );
  };
  ...
  const handleUpdateTypeName = (typeId: string, newName: string) => {
    setState((prev) =>
      DocumentDatabaseService.updateTypeName(prev, typeId, newName),
    );
  };
  ...
  const handleAddDocumentType = () => {
    setState((prev) =>
      DocumentDatabaseService.addDocumentType(prev, selectedCategoryId),
    );
  };
  ...
  const handleToggleRequired = (typeId: string, columnId: string) => {
    setState((prev) =>
      DocumentDatabaseService.toggleRequired(prev, typeId, columnId),
    );
  };
  ...
  const handleToggleBasicInfo = (typeId: string, columnId: string) => {
    setState((prev) =>
      DocumentDatabaseService.toggleBasicInfo(prev, typeId, columnId),
    );
  };
  ...
  const handleToggleTableDisplay = (typeId: string, columnId: string) => {
    setState((prev) =>
      DocumentDatabaseService.toggleTableDisplay(prev, typeId, columnId),
    );
  };
  ...
  // 現在選択されているカテゴリを取得
  const selectedCategory = state.documentCategories.find(
          ...
          <div className='space-y-4'>
              ...
              <div key={type.id} className='space-y-4'>
                ...
                <div className='flex items-center gap-3'>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleUpdateTypeName(type.id, e.target.value)
                    }
                    className='text-primary h-auto border-0 bg-transparent p-0 text-xl font-semibold shadow-none focus:ring-1 focus:ring-blue-500'
                ...
                <DatabaseColumnSetting
                  ...
                  onUpdateColumn={(columnId, updates) =>
                    handleUpdateColumn(type.id, columnId, updates)
                  }
                  onDeleteColumn={(columnId) =>
                    handleDeleteColumn(type.id, columnId)
                  }
                  onAddColumn={() => handleAddColumn(type.id)}
                  onToggleRequired={(columnId) =>
                    handleToggleRequired(type.id, columnId)
                  }
                  onToggleBasicInfo={(columnId) =>
                    handleToggleBasicInfo(type.id, columnId)
                  }
                  onToggleTableDisplay={(columnId) =>
                    handleToggleTableDisplay(type.id, columnId)
                  }
                />
              </div>
            ))}
            ...
            <div className='flex justify-center pt-4'>
              <Button
                ...
                variant='outline'
                size='lg'
                className='flex items-center gap-2 border-2 border-dashed border-gray-300 px-8 py-6 text-gray-600 hover:border-gray-400 hover:bg-gray-50 hover:text-gray-700'
              >
