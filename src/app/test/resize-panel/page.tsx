...
import { ResizableLayout, ResizablePanel, ResizableHandle, Button } from '@/shared';
import { 
  twoColumnConfig, 
  threeColumnConfig, 
  fourColumnConfig, 
    ...
    for (let i = 0; i < config.panels.length; i++) {
      ...
      panels.push(
        <ResizablePanel key={`panel-${i}`} index={i}>
          <div className="h-full bg-gradient-to-br from-blue-50 to-blue-100 p-4 border border-blue-200">
            <div className="font-bold text-blue-800 mb-2">
              パネル {i + 1}
            </div>
            <div className="text-sm text-blue-600">
              <div>Index: {i}</div>
              <div>初期サイズ: {config.direction === 'horizontal' 
                ? config.panels[i].initialWidth 
                : config.panels[i].initialHeight}%</div>
              <div>最小: {config.direction === 'horizontal' 
                ? config.panels[i].minWidth 
                : config.panels[i].minHeight}%</div>
              <div>最大: {config.direction === 'horizontal' 
                ? config.panels[i].maxWidth 
                : config.panels[i].maxHeight}%</div>
            </div>
            <div className="mt-4 p-2 bg-white rounded border">
              <p className="text-xs text-gray-600">
                このパネルをリサイズできます。
                {config.direction === 'horizontal' ? '左右' : '上下'}のハンドルをドラッグしてください。
              </p>
            </div>
          </div>
        </ResizablePanel>
    ...
    <div className="h-screen flex flex-col bg-gray-50">
      ...
      <div className="flex-shrink-0 p-4 bg-white border-b shadow-sm">
        ...
        <div className="mt-2 text-sm text-gray-600">
          ...
          {' - '}方向: {configMap[currentConfig].config.direction === 'horizontal' ? '水平' : '垂直'}
        </div>
      </div>
      ...
      {/* フッター情報 */}
      <div className="flex-shrink-0 p-4 bg-white border-t text-xs text-gray-500">
        <div className="flex gap-8">
          <div>
            <strong>使い方:</strong> ハンドル（グレーの縦線/横線）をドラッグしてパネルをリサイズ
          </div>
          <div>
