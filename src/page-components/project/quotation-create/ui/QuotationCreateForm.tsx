...
import { QuotationFormData, EstimateInformation } from '../model';
...
export function QuotationCreateForm({
  ...
  const [blueprints, setBlueprints] = useState<BlueprintDetailDataInterface[]>(
    projectDetailData.blueprints,
  );
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedBlueprint, setSelectedBlueprint] =
    useState<BlueprintDetailDataInterface | null>(null);

  const updateFormField = (field: keyof QuotationFormData, value: unknown) => {
    setFormData({ ...formData, [field]: value });
  };

  // 見積もりボタンクリック時の処理
  const handleEstimate = (blueprint: BlueprintDetailDataInterface) => {
  ...
  const handleSaveEstimate = (
    blueprintId: string,
    estimateData: EstimateInformation,
  ) => {
    // 図面データを更新
    setBlueprints((prev) =>
      prev.map((blueprint) =>
        blueprint.id.toString() === blueprintId
          ? {
              ...blueprint,
              estimate_information: {
                ...blueprint.estimate_information,
                ...estimateData,
              },
            }
          : blueprint,
  ...
  // GalleryView設定を生成
  const galleryConfig =
    createQuotationCreateBlueprintGalleryConfig(handleEstimate);
  ...
  return (
    <div className='h-full space-y-8 overflow-y-auto p-6'>
        ...
        <div className='mb-6 grid grid-cols-12 gap-4'>
          ...
          <div className='col-span-2'>
            ...
            <Select
              ...
              onValueChange={(value) => updateFormField('honorific', value)}
            >
              <SelectTrigger className='w-full'>
          ...
          <div className='col-span-6'>
            ...
            <Input
              ...
              onChange={(e) =>
                updateFormField('quotationNumber', e.target.value)
              }
            />
          </div>
        </div>
        ...
        <div className='grid grid-cols-2 gap-4'>
          <div>
            ...
            <Select
              ...
              onValueChange={(value) => updateFormField('issueDate', value)}
            >
              <SelectTrigger className='w-full'>
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(value) => updateFormField('validUntil', value)}
            >
              <SelectTrigger className='w-full'>
      ...
      {/* 中部: 図面ギャラリー */}
      <div>
        <h2 className='mb-4 text-lg font-semibold'>図面別見積もり</h2>
        <div className='min-h-[400px]'>
          <GalleryView data={blueprints} config={galleryConfig} />
        </div>
      </div>

      {/* 下部: 備考 */}
      <div>
        <h2 className='mb-4 text-lg font-semibold'>備考</h2>
        <Textarea
