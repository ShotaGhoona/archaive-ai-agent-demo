...
export const calculateUnitCosts = (state: EstimateState): EstimateCosts => {
  // 材料費単価 = 全材料の単価合計
  const materialUnitCost = state.materials.reduce(
    (sum, item) => sum + item.price,
    0,
  );

  // 工程費単価 = 全工程の(時間 × チャージ)の合計
  const processUnitCost = state.processes.reduce(
    (sum, item) => sum + item.timeMinutes * item.chargeRate,
    0,
  );

  // 段取り工程費用単価 = 全段取り項目の単価合計
  const setupUnitCost = state.setupCosts.reduce(
    (sum, item) => sum + item.price,
    0,
  );
  ...
  // 最終見積金額
  const finalTotal =
    materialTotalCost + processTotalCost + setupTotalCost + otherCost;
