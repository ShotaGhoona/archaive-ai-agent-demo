        ...
        <div
          className={`rounded-lg border-2 border-dashed p-6 text-center transition-all duration-200 ${
            dragOverTarget === 'new-project'
              ...
              : 'hover:border-primary hover:bg-primary/5 border-gray-300 bg-gray-50'
          }`}
        ...
        >
          <div className='flex flex-col items-center gap-3'>
            ...
            <div className='space-y-1'>
              ...
              <p className='text-xs text-gray-500'>新案件作成</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <>
          ...
          <ScrollArea className='flex-1'>
            <div className='space-y-3'>
              {projects.map((project) => (
                <Card
                  key={project.id}
                  className={`border-2 border-dashed p-0 transition-all duration-200 ${
                    dragOverTarget === project.id
                      ? 'border-primary bg-primary/5 scale-105 shadow-lg'
                      : 'hover:border-primary border-gray-300 hover:shadow-md'
                  }`}
                >
                  <CardContent className='p-4'>
                    ...
                    {/* ドロップエリア */}
                    <div
                      className={`flex min-h-[80px] flex-col items-center justify-center rounded-md border border-dashed p-4 text-center transition-all duration-200 ${
                        dragOverTarget === project.id
                    ...
                    >
                      {project.fileCount > 0 ? (
                        ...
                        <div className='space-y-2'>
                          <div
                            className={`mx-auto h-6 w-6 rounded border-2 border-dashed ${
                              dragOverTarget === project.id
                                ? 'border-primary'
                                : 'border-gray-300'
                            }`}
          ...
          <div className='flex-shrink-0 border-t pt-4'>
            <Link href='/project/PJ-2024-001/basic-information'>
              <Button onClick={onBatchRegister} className='w-full' size='lg'>
