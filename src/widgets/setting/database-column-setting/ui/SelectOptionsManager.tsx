...
export function SelectOptionsManager({
  ...
  // 変更検知のためのuseEffect
  useEffect(() => {
    const originalOptionsString = JSON.stringify(
      options.sort((a, b) => a.id.localeCompare(b.id)),
    );
    const editingOptionsString = JSON.stringify(
      editingOptions.sort((a, b) => a.id.localeCompare(b.id)),
    );
      ...
      <Popover open={isOpen} onOpenChange={handleOpenChange}>
        <PopoverTrigger asChild>
          <Button
            variant='outline'
            size='lg'
            className={`flex items-center gap-1 text-xs ${disabled ? 'cursor-not-allowed bg-gray-50 text-gray-600' : ''}`}
            disabled={disabled}
          >
        ...
        <PopoverContent className='w-80 p-3' align='start'>
          <div className='space-y-3'>
                ...
                >
                  <div className='flex flex-1 items-center gap-2'>
                    ...
                    <Popover
                      ...
                      onOpenChange={(open) =>
                        setColorPickerOptionId(open ? option.id : null)
                      }
                    >
                      <PopoverTrigger asChild>
                        <button
                          className='h-6 w-6 cursor-pointer rounded-sm border border-gray-300 transition-colors hover:border-gray-500'
                        ...
                        <div className='grid grid-cols-4 gap-1'>
                          {PRESET_COLORS.map((color) => (
                            <button
                              ...
                              onClick={() =>
                                handleColorChange(option.id, color)
                              }
                            />
                          ))}
                        </div>
                    ...
                    {editingOptionId === option.id ? (
                      ...
                      <span
                        className='hover:text-primary flex-1 cursor-pointer text-sm'
                  ...
                  <Button
                    ...
                    className='h-5 w-5 p-0 text-gray-400 opacity-0 transition-opacity group-hover:opacity-100 hover:text-red-500'
                  >
            ...
            <Button
              ...
              variant='outline'
              className='w-full border-2 border-dashed border-gray-300 py-2 text-gray-600 hover:border-gray-400 hover:bg-gray-50 hover:text-gray-700'
            >
